...
describe('cache-storage', function () {
    ...
    it('addImage adds images to cache', function () {
        var cache = createMockContext('http://example.com', { proxy: null }).cache;
    ...
    it('addImage should not add duplicate entries', function () {
        var cache = createMockContext('http://example.com').cache;
        cache.addImage('http://example.com/test.jpg');
        cache.addImage('http://example.com/test.jpg');
    ...
    describe('svg', function () {
        it('should add svg images correctly', function () {
            var cache = createMockContext('http://example.com').cache;
        ...
        it('should omit svg images if not supported', function () {
    ...
    describe('cross-origin', function () {
        it('addImage should not add images it cannot load/render', function () {
            var cache = createMockContext('http://example.com', {
                proxy: undefined
            }).cache;
            cache.addImage('http://html2canvas.hertzen.com/test.jpg');
            (0, assert_1.deepStrictEqual)(images.length, 0);
        });
        it('addImage should add images if tainting enabled', function () {
            var cache = createMockContext('http://example.com', {
                allowTaint: true,
                proxy: undefined
            }).cache;
        ...
        it('addImage should add images if cors enabled', function () {
            var cache = createMockContext('http://example.com', { useCORS: true }).cache;
            cache.addImage('http://html2canvas.hertzen.com/test.jpg');
            (0, assert_1.deepStrictEqual)(images.length, 1);
            (0, assert_1.deepStrictEqual)(images[0].src, 'http://html2canvas.hertzen.com/test.jpg');
            (0, assert_1.deepStrictEqual)(images[0].crossOrigin, 'anonymous');
        });
        it('addImage should not add images if cors enabled but not supported', function () {
        ...
        it('addImage should not add images to proxy if cors enabled', function () {
            var cache = createMockContext('http://example.com', { useCORS: true }).cache;
            cache.addImage('http://html2canvas.hertzen.com/test.jpg');
            (0, assert_1.deepStrictEqual)(images.length, 1);
            (0, assert_1.deepStrictEqual)(images[0].src, 'http://html2canvas.hertzen.com/test.jpg');
    ...
    it('addImage should add an inlined image', function () { return __awaiter(void 0, void 0, void 0, function () {
        var cache, inlinedImg;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    cache = createMockContext('http://example.com', { imageTimeout: 10 }).cache;
                    inlinedImg = "data:image/gif;base64,R0lGODlhEAAOALMAAOazToeHh0tLS/7LZv/0jvb29t/f3//Ub/\n/ge8WSLf/rhf/3kdbW1mxsbP//mf///yH5BAAAAAAALAAAAAAQAA4AAARe8L1Ekyky67QZ1hLnjM5UUde0ECwLJoExKcpp\nV0aCcGCmTIHEIUEqjgaORCMxIC6e0CcguWw6aFjsVMkkIr7g77ZKPJjPZqIyd7sJAgVGoEGv2xsBxqNgYPj/gAwXEQA7";
                    cache.addImage(inlinedImg);
                    return [4 /*yield*/, cache.match(inlinedImg)];
                case 1:
                    _a.sent();
                    return [2 /*return*/];
            }
